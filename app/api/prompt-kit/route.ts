...
The test: can a [job title] paste this into ChatGPT right now, with zero editing, and get something they would actually use at work? If they would have to stop and think about what to fill in first, or rewrite the prompt before using it, it is too generic. Build tighter.`;

  const toolVarianceInstruction = challenge === "I'm not sure which AI tool to use for what"
    ? `\n\nTool variety rule: This user's stated challenge is not knowing which AI tool to use for what. In at least 3 of the 12 prompts, name the best tool for that specific task rather than defaulting to "ChatGPT or Claude." Use the right tool for the job: Perplexity for live web research, Otter.ai for meeting transcription and notes, Claude for long documents, nuanced writing, and analysis, Gemini for Google Workspace tasks, ChatGPT for quick drafts and brainstorming. Name the tool in the prompt text itself so they build a real mental map of the AI ecosystem, not just more prompts for the same two tools.`
    : "";

  const complexityInstruction = aiUsage === "I haven't started using AI tools yet"
    ? `Complexity level: BEGINNER. Short prompts only. Simple context-setting, one task per prompt, no jargon, no advanced techniques. These should feel completely approachable to someone opening ChatGPT for the first time. Tool guidance: reference ChatGPT consistently throughout — do not introduce other tools. One clear path, no decision fatigue. Their first win matters more than a complete picture of the AI ecosystem.`
    : aiUsage === "I try it sometimes, but results feel hit or miss"
    ? `Complexity level: EARLY. Include one concrete tip in each "why it works" explanation about what specifically makes this prompt work better than what they have likely been doing. Help them see the difference. Tool guidance: default to ChatGPT or Claude. You may introduce one alternative tool if there is a strong, obvious reason — for example, Perplexity for a research-heavy prompt. No more than one alternative across the full kit. When you introduce it, include one sentence in the "why it works" field explaining why that specific tool is better for that task.`
    : aiUsage === "I use it a few times a week"
    ? `Complexity level: INTERMEDIATE. Include explicit format instructions in every prompt. Model the context-first habit — every prompt should open with a sentence about who the user is and what they are working on before the task. Tool guidance: recommend the best tool for the job when there is a clear reason. Name alternatives in the prompt text itself: Perplexity for live research, Otter.ai for meeting transcription, Claude for long documents and nuanced analysis, Gemini for Google Workspace tasks. Limit to 2-3 tool alternatives across the full kit — enough to expand their toolkit without overwhelming them.`
    : `Complexity level: ADVANCED. Include constraints, specificity, and chaining hints. Prompts can be more sophisticated — introduce role-priming, output constraints, and multi-step prompts that build on each other. Tool guidance: name the best tool for every relevant task. Do not default to ChatGPT or Claude when a more specific tool is clearly better. Full ecosystem is fair game: Perplexity for research, Otter.ai for transcription, Claude for long-context analysis, Gemini for Google Workspace, ChatGPT for quick iteration and brainstorming, Gamma for AI-native presentations, Notion AI for knowledge management. Name the tool in the prompt text itself so they build a real mental map of the AI ecosystem.`;
