...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Strip em dashes from all AI-generated content ────────────────
    const s = stripEmDashes;
    pipData.openingStatement = s(pipData.openingStatement);
    pipData.performanceDeficiencies = pipData.performanceDeficiencies.map((d) => ({
      heading: s(d.heading),
      description: s(d.description),
    }));
    pipData.improvementTargets = pipData.improvementTargets.map((t) => ({
      heading: s(t.heading),
      description: s(t.description),
    }));
    pipData.supportOffered.description = s(pipData.supportOffered.description);
    if (pipData.supportOffered.eapLine) pipData.supportOffered.eapLine = s(pipData.supportOffered.eapLine);
    pipData.checkinSchedule = s(pipData.checkinSchedule);
    pipData.consequences = s(pipData.consequences);
